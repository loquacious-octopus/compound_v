...
from miner_reference.production_ensemble import create_generator
...
# Per the API Spec: lowercase alphanumeric, unique within a round. Live
# prompt pools have also included UUID-style hyphenated stems, so accept
# lowercase hyphens as well.
...
async def _run_generation(state: MinerState, seed: int) -> None:
    ...
    2. Produces a result for every prompt. Runs even when the benchmark failed so the pod can
    ...
    try:
        ...
        if hasattr(generator, "generate_batch"):
            def record_progress(stem: str, generated) -> None:
                if generated.source:
                    state.results[stem] = generated.source
                else:
                    state.failed[stem] = generated.error or "generation returned no source"
                state.progress = len(state.results) + len(state.failed)
            ...
            for i, prompt in enumerate(state.prompts):
                try:
                    ...
                    if generated.source:
                        state.results[prompt.stem] = generated.source
                    else:
                        state.failed[prompt.stem] = generated.error or "generation returned no source"
...
async def _run_gpu_benchmark(state: MinerState) -> None:
    ...
    if os.environ.get("SKIP_GPU_BENCHMARK", "").strip().lower() in {"1", "true", "yes", "on"}:
        logger.warning("SKIP_GPU_BENCHMARK is set — skipping GPU benchmark")
        return
    ...
    try:
        ...
        logger.warning(f"GPU benchmark failed to run; marking hardware degraded: {exc}")
        state.gpu_degraded = True
        state.benchmark_payload = {"benchmark": "gpu_health", "all_passed": False, "error": str(exc)}
        return
